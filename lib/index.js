...
export default class Gestures extends Component {
  ...
  reset = (scaleStyle, leftValue, topValue) => {
    const { transform, top, left } = this.prevStyles;

    this.dragStyles = { left: leftValue >= 0 ? leftValue : left, top: topValue >= 0 ? topValue : top };
    this.pinchStyles = { transform : scaleStyle || transform };
